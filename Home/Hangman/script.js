...
async function game(difficulty) {
    const word = await initializeAI(difficulty); // Fetch the word from AI
    const maxAttempts = 6;
    ...
    async function createHelp_for_Dummies(difficulty, word) {
        ...
        let prompt2;
        if (difficulty === "Hard") {
            prompt2 = `Give me a hint for the word "${word}" that describes its opposite meaning or function. For example, if the word is "fenster," the response should be: "Etwas, das Licht blockiert und keinen Durchblick erlaubt." Respond with only the hint and nothing else.`;
        } else {
            prompt2 = `Give me a hint for the word "${word}" that is related to the word. For example, if the word is "fenster," the response should be: "Teil eines Hauses wo man durchschauen kann" and nothing else.`;
        }
    ...
    updateWordDisplay();
    updateAttemptsDisplay();
    createHelp_for_Dummies(difficulty, word); 
}
